...
const AppointmentSlots = () => {
    const {state, model, currentDaySlots, AppointmentAvailableFutureDates} = useSlotModelView();


    const tittle = model.isRescheduleMode ?
        appointmentSlotsResources.selectDate(model.haveSlotBookedInFuture()).titleReschedule
        : appointmentSlotsResources.selectDate(model.haveSlotBookedInFuture()).title;
    return (
        <div
            className="w-full max-w-[900px] h-[90vh] md:h-[800px] mt-4 rounded-sm shadow-xl flex flex-col overflow-hidden bg-white mx-auto">
            {/* Header */}
            <SlotHeader
                branchName={state.branchName}
                distance={state.distance}
                onBack={model.backToBranch}
                mode={model.isRescheduleMode?"reschedule":"default"}
            />
            {

                AppointmentAvailableFutureDates ? (
                <>
                    ...
                    {model.isRescheduleMode && (
                        ...
                        >
                            ...
                            <p style={{...typography.bodySmall, color: colors.textSecondary, margin: 0}}>
                                {appointmentSlotsResources.rescheduleHeader.currentlyScheduled}
                                <strong>{model.formatCurrentAppointmentDate()}</strong>
                            </p>
                            <p style={{...typography.bodySmall, color: colors.textMuted, margin: 0}}>
                                {appointmentSlotsResources.rescheduleHeader.service}
                                <strong>{model.rescheduleData?.serviceType}</strong>
                            </p>
                        </div>
                    )}
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                        ...
                        <div className="mb-6">
                            <h4 style={{
                                ...typography.h4,
                                color: model.haveSlotBookedInFuture()?colors.red:colors.primary
                            }}>{tittle}</h4>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                            {currentDaySlots.map((slot) => (
                                <SlotButton
                                    ...
                                    isFullyBooked={model.isFullyBooked(slot)}
                                    isBlocked={model.isBlocked(slot)}
                                    haveSlotBookedInFuture={model.haveSlotBookedInFuture()}
                                />
                            ))}
                        </div>
                    </div>
                    ...
                    (<EmptySlots noAppointmentDatesInNearFuture={!AppointmentAvailableFutureDates}/>)
